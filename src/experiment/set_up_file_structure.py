...
def main():
    ...
    for family in os.listdir(path):
        if os.path.isdir(f'{path}/{family}'):
            ...
            shutil.move(f'{path}/{family}/total_demand.csv', f'{path}/{family}/input/demand.csv')
            ...
            cost_allocation = {'OnDemand': [0 for i in range(final_t)], 'RAll': [0 for i in range(final_t)], 'RPartialUp': [0 for i in range(final_t)], 'RPartialHr': [0 for i in range(final_t)], 'RNo': sp_values}
